...
As displayed in Eq.~(\ref{eq:drydep_velo}), the dry deposition computation is subdivided into contributions from three different resistances. The main idea of a mosaic approach is to calculate these resistences dependent on the fraction of land surface types $k$ in each grid cell: $R_a^k$,$R^{i,k}_b$, and $R^{i,k}_c$.

\subsubsection{Aerodynamic resistance}
\label{subsubsec:Ra}
In general, the aerodynamic resistance is describing the turbulent transport of any substance down to the surface. To derive $R_a^k$, \citet{WASP:Simpson2003,ACP:Simpson2012} compute a local friction velocity at reference height $z_\mathrm{ref}$ \citep[Eq.~(52),][]{ACP:Simpson2012}
\begin{equation}
...
\end{equation}
with an average wind speed $\overline{u}(z_\mathrm{ref})$, the K\'{a}rm\'{a}n constant $\kappa = 0.40$, the integrated stability equation for momentum $\Psi_m$ \citep[e.g.,][]{Garratt1992}, a grid average Obukhov length $L$, deplacement height $d_k$, and surface height $z_0^k$ ($d_k=0.78\cdot h_k(\mathrm{lat})$, $z_0^k=0.07\cdot h_k(\mathrm{lat})$ for forests, $d_k=0.7\cdot h_k(\mathrm{lat})$, $z_0^k=0.1\cdot h_k(\mathrm{lat})$ for vegetation other than forests). We have chosen a model level where $\overline{z}_\mathrm{ref}\approx 45\,\unit{m}$. From Eq.~(\ref{eq:local_fric_vel}), a local Obukhov length $L_k$ is found \citep[Eq.~(8),][]{ACP:Simpson2012}:
\begin{equation}
  L_k = - \frac{\rho c_p T_\mathrm{2m} u_*^k}{\kappa g H}.
\end{equation}
Herein, $H$ is the sensible heat flux, $g$ is the standard gravitational acceleration, $c_p$ the specific heat capacity, and $T_{2m}$ the $2\,\unit{m}$ temperature. With this we get \citep[Eq.~(8.8),][]{WASP:Simpson2003}
\begin{equation}
...
\end{equation}
with the integrated stability equation for heat $\Psi_h$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Quasi-laminar layer resistance}
\label{subsubsec:Rb}
The quasi-laminar layer resistance $R_b^{i,k}$ is species specific and differs only over land and ocean surfaces.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
For the temperature dependent dynamic viscosity of air $\mu(T)$, we chose a linear fit to Sutherland's law through the origin within the temperature range $\{T \in \mathbb{R} | (243.15 < T < 313.15)\,\unit{K}\}$: $\mu(T) = 6.2\cdot 10^{-8}\,\unit{kg\,m^{-1}\,s^{-1}\,K^{-1}} \cdot T$. But despite its rough accuracy, we found that the choice of $\mu(T)$ has no effect on $\overline{R}_b^{i,k}$ (Supplement~S.1: Figs.~S1--S2). In Eq.~(\ref{eq:kinematic_viscosity}), $\rho$ is substituted by the air density using the ideal gas law. $P_0$ is the surface pressure, as $T$ the $2\,\unit{m}$ temperature is chosen, and $R_\text{air}$ is the universal gas constant for air.
