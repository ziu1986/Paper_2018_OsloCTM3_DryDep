...
\end{figure*}
%
As suggested by \citet{ACP:Hardacre2015}, we retrospectively separate ozone dry deposition velocities with respect to surface types. Based on a CLM~2 average dynamic land surface map, we generated masks for 9 different surface types (Fig.~\ref{fig:pft_landsurface}a) and used these to select gridboxes with a high percentage of these surface types, ranging from a meager $22\,\unit{\%}$ for tundra in the southern hemisphere to $98\,\unit{\%}$ regarding ocean. From this CLM~2 based map, it was not possible to select gridboxes with $100\,\unit{\%}$ for each surface type. Since we have not perform a full unfolding on the data, the results should be treated with slight caution. In Fig.~\ref{fig:mmm_drydep_season_pft}, the seasonal cycles of dry deposition velocities are shown for the nine surface categories. The patterns and absolute numbers differ between the schemes and the multi-model-mean. Exceptually large is the divergence for desert regions. The multi-model-mean displays a very pronounced seasonal cycle ($0.15\,\unit{cm\,s^{-1}} \leq v^\chem{O_3}_\text{desert}\leq 0.10\,\unit{cm\,s^{-1}}$). The Wesely scheme as well as the EMEP scheme do not indicate any seasonal cycle. The estimated dry deposition velocities over desert regions are consistent with the previously mentioned baseline values of ozone dry deposition,  about one order of magnitude lower than in the EMEP scheme than in the Wesely scheme. From measurements of ozone fluxes in the Sahara desert, $v^\chem{O_3}_\text{desert, day} = 0.1\,\unit{cm\,s^{-1}}$, $v^\chem{O_3}_\text{desert, night} = 0.04\,\unit{cm\,s^{-1}}$, and $\left<v^\chem{O_3}_\text{desert}\right> = 0.065\,\unit{cm\,s^{-1}}$ have been deduced \citep{AE:Gusten1995}. This implies, that ozone dry deposition over desert regions is highly overestimated in the Wesely scheme, while it may be underestimated in the EMEP scheme. Similarly, the dry deposition velocities over water differ. From measurements over the ocean a value of $v^\chem{O_3}_\text{water} = 0.019\,\unit{cm\,s^{-1}}$ has been deduced \citep{JGR:Helmig2012}. In a comparison of various dry deposition mechanisms over ocean waters implemented in their model, \citet{ACP:Luhar2017} give a range of $0.018\,\unit{cm\,s^{-1}} \leq \left<v^\chem{O_3}_\text{water}\right> \leq 0.039\,\unit{cm\,s^{-1}}$.
%
\begin{figure*}[t]
