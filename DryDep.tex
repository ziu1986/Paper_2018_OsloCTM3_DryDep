...
\end{equation}
Herein, $H$ is the sensible heat flux, $g$ is the standard gravitational acceleration, $c_p$ the specific heat capacity, and $T_{2m}$ the $2\,\unit{m}$ temperature. With these, we get \citep[Eq.~(8.8),][]{WASP:Simpson2003}
\begin{equation}
...
$R_\text{gs}^{\chem{O_3},k}$ is tabulated and we correct for temperature by $F_T$ and for snow cover fraction:
\begin{equation}
  \frac{1}{R_\text{gs}^{\chem{O_3},k}} = \frac{1-f_\text{snow}}{\hat{R}_\text{gs}^{\chem{O_3},k}} + \frac{f_\text{snow}}{R_\text{snow}^\chem{O_3}}.
\end{equation}
...
\end{equation}
wherein $\text{LAI}$ is the one-sided leaf area index taken from ISLSCP2 FASIR (zero for non-vegetated surfaces), $g_\text{sto}$ the leaf-level stomatal conductance, and $G_\text{ns}$ the bulk non-stomatal conductance.\\
