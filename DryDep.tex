...
\section{Evaluation}
\label{sec:eval}
In this section, we present results from a manifold of OsloCTM3 model runs testing different parameters of the EMEP scheme. We focus on the change in total ozone dry deposition, ozone dry deposition velocities, and surface ozone concentration related to the stomatal conductance part of the surface resistance. We evaluate the results in the context of the multi-model comparison and observations
in\citet{ACP:Hardacre2015} (Section~\ref{subsec:model}-\ref{subsec:obs}). The OsloCTM3 is driven by meteorological input fields from \href{https://www.ecmwf.int/en/forecasts/documentation-and-support/evolution-ifs/cycle-38r1-summary-changes}{ECMWF - OpenIFS version cy38r1nc4}. CEDS historical emission inventory is used for anthropogenic emissions, while biomass burning is covered by NASA's Global Fire Emissions Database, Version4 (GFEDv4). In the following, we will discuss the various model sensitivity studies.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Sensitivity tests}
\label{subsec:sens}
A summary of specifications of all simulations discussed in this section is given in Table~\ref{tab:simsum}. For simplicity, we only list the tested parameters. An x in the table denotes that the model was run exactly in the configuration as has been decribed in Section~\ref{sec:model_des}. With respect to the sensitivity on light (PPFD), $f_\text{light}\equiv 1$ has been tested. Regarding the phenology, we have tested three different options: No phenology ($f_\text{phen}\equiv 1$), phenology including / excluding the Tropics (TR). Diverging from \citet{ACP:Simpson2012}, the simulations inititally ran with the uppermost soil water level (SWVL1). One sensitivity test was conducted using SWVL4. The meteorological year used in all simulations is 2005. Most of the simulations were run with CEDS emissions for 2014. One test has been conducted to look at the impact of the emissions.
\begin{table*}[t]
