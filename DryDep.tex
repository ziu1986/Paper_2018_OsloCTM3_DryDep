...
\end{equation}
with $\beta = \frac{T_\text{max}-T_\text{opt}}{T_\text{opt}-T_\text{min}}$. The parameters $T_\text{min}$, $T_\text{max}$ and $T_\text{opt}$ are tabulated for various land surface types. All parameters used in the EMEP scheme can be found in the supplementary to \citet{ACP:Simpson2012} or in Supplement~S.2. Since $f_\text{T}$ becomes negative outside its range defined by $T_\text{min}$, $T_\text{max}$, we impose a lower limit of $0.01$. The water vapor deficit (VPD) is proportional to the saturation partial pressure of water ($P_\chem{H_2O}^s$) and relative humidity ($\text{RH}$)
%
\begin{equation}
  \text{VPD} = P_\chem{H_2O}^s \cdot (1 - \text{RH}/100).
\end{equation}
%
Using tabulated values of $f_\text{min}$, $D_\text{min}$, $D_\text{max}$, the water vapor pressure deficit penalty factor can be computed:
%
\begin{equation}
  f_\text{D} = f_\text{min}+(1-f_\text{min}) \cdot \frac{D_\text{min} - \text{VPD}}{D_\text{min}-D_\text{max}}.
\end{equation}
%
The penalty factor with respect to available soil water ($\text{SW}$) is defined as 
%
\begin{equation}
  f_\text{SW} =
  \begin{cases}
...
\end{equation}
%
The parameters $\phi_a$, $\phi_b$, $\phi_c$, and $\phi_d$ define start or end points in the five phases of phenology in the EMEP scheme, while $\phi_e$, $\phi_f$, $\phi_\text{AS}$, and $\phi_\text{AE}$ control the temporal timing (Fig.~\ref{fig:phenology}). If GDAY is zero we are most likely in Arctic regions and there is no vegetation anyway, therefore $f_\text{phen}=0$. Since the EMEP scheme is tuned to NH mid latitudes, this phenology does not apply to the tropics, after evaluation of the model results (Section~\ref{subsec:sens}), we decided to set $f_\text{phen}=1$ if GLEN is greater or equal 365 which is the case in the tropics.
%
\begin{figure}[t]
...
\end{figure}
%
Light in the wavelength band $(400-700)$\,\unit{nm} to which the plant chlorophyll is sensitive is called photosynthetic active radiation (PAR). In the EMEP parameterization, an integrated photon flux -- photosynthetic photon flux density (PPFD) -- is used to mimic the time lag of the opening and closing of stomata with respect to light instead of a simple day-on/night-off treatment:
\begin{equation}
...
\end{equation}
Herein $R$ is the universal gas constant. With this the total gridbox stomatal conductance $G_\text{sto}$ is computed weighted by the land surface type fraction $f_L(N)$:
\begin{equation}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \begin{cases}
      ...
      \begin{cases}
        \emph{if} \quad \emph{dry} & \max{(a_1^\text{hydrophob.}, 0.008\cdot\frac{\text{SAI}}{10})}\\
        \emph{if} \quad \emph{wet} & 1.5\cdot\max{(a_1^\text{hydrophob.}, 0.008\cdot\frac{\text{SAI}}{10})}\\
    \end{cases}\\
    \end{cases}\\
...
The original EMEP scheme uses fixed start of growing season (SGS) and end of growing season (EGS) at $50\,\unit{^\circ N}$ together with leapsrates ($d_\text{SGS}$, $d_\text{EGS}$, $\nabla d_\text{SGS}$, and $\nabla d_\text{EGS}$ in Supplement~S.2) to define phenology and dry deposition over agricultural areas. However this parameterization fails in a global model, therefor another latitude dependent parameterization is needed. First, we have used a parameterization as implemented in the Sparse Matrix Operational Kernel Emissions -- Biogenic Emission Inventory System (SMOKE-BEIS) model. SMOKE-BEIS has fixed values for SGS and EGS for all regions but NH mid latitudes ($23\,\unit{^\circ} < \text{lat} < 65\,\unit{^\circ}$), where it uses leapsrates of $\nabla d_\text{SGS} = 4.5$ and $\nabla d_\text{EGS} = 3.3$. This parameterization works well, if the focus of the scientific work lies on mid latitudes and North America. In Europe, however, most of northern Scandinavia has no vegetation period according to this parameterization. 

In agriculture, there are differing empirical rules to estimate the SGS and EGS. The simplest assumption is that greening starts after 5 consecutive days with a daily average temperature above 5\,\unit{^\circ C} and vice versa for EGS. If, within these 5 days, the temperature drops below / rises above 5\,\unit{^\circ C}, the counters are reset. With respect to model performance, we decided to preprocess our meteorological input date using this 5\,\unit{^\circ C}-days criteria for $45\,\unit{^\circ} < \text{lat} < 85\,\unit{^\circ}$ in the NH and $35\,\unit{^\circ} < \text{lat} < 65\,\unit{^\circ}$ in the SH. In all other cases and where the 5\,\unit{^\circ C}-days criterion fails, we use the SMOKE-BEIS parameterization. 

Instead of SGS and EGS, we use the derived parameters day of greening season (GDAY), which is the time elapsed starting at the SGS, and the total length of the greening season (GLEN), which is the time between EGS and SGS. Example maps of the computed GLEN are shown in Supplement~S.4.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Photosynthetic photon flux density (PPFD)}
From OpenIFS an accumulated photosynthetic active radiation (PAR) is available, but needs to be deaccumulated. Strictly speaking, since this field is already integrated over the whole spectrum, it is not the actual PAR but PPFD. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%      
\section{Evaluation}
\label{sec:eval}
