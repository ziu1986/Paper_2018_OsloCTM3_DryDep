...
\subsection*{Land}
Over land we use the definition \citep[Eq.~(53),][]{ACP:Simpson2012}
\begin{equation}
...
%
\subsection*{Ocean}
Over ocean we use the definition \citep[Eq.~(54),][]{ACP:Simpson2012}
\begin{equation}
  R_b^i = \frac{1}{\kappa u_*}\cdot\ln\left({\frac{z_0}{D_i}\cdot \kappa u_*}\right)
\end{equation}
with an imposed a lower threshold of 10\,\unit{sm^{-1}} and an upper limit of 1000\,\unit{sm^{-1}}. The computation of roughness length $z_0$ over ocean is divided into a \emph{calm} and a \emph{rough} sea case, where wind speeds above 3\,\unit{ms^{-1}} are considered as boundary between both cases. Calm sea follows \citet{Hinze1975,Garratt1992} with a slightly increased coefficient:
\begin{equation}
  z_0^\text{calm} = \text{min}\left(2\cdot10^{-3}, 0.135 \cdot \frac{\nu}{u_*}\right).
\end{equation}
The kinematic viscosity of air $\nu = \mu/\rho$ can be computed from the absolute viscosity of air $\mu$, and substituting $\rho$ with surface pressure $P_0$ using the ideal gas law, 2\,\unit{m} temperature $T_\text{2M}$, and the universal gas constant for air $R_\text{air}$
\begin{equation}
  \nu = \frac{T_\text{2M}}{\frac{P_0}{T_\text{2M}\cdot R_\text{air}}} \cdot 6.2\cdot 10^{-8}\,\unit{kg m^{-1} s^{-1} K^{-1}}.
\end{equation}
...
In the EMEP scheme, non-stomatal conductances are explicitly calculated for \chem{O_3}, \chem{SO_2}, \chem{HNO_3}, and \chem{NH_3}. For all other species, an interpolation between \chem{O3} and \chem{SO_2} values is carried out.
...
\end{equation}
$\text{SAI}(N)$ is the surface area index for vegetation type $N$, which is $\text{LAI}$ plus some value representing cuticles and others surfaces. The external leaf resistance $r_\text{ext} = 2000\,\unit{sm^{-1}}\cdot F_T$. Below $-1\,\unit{^\circ C}$,s a temperature correction factor $F_T$ is used
\begin{equation}
  F_T = \exp{(-2\cdot(1+T_0))}.
\end{equation}
It ranges between 1--2.
