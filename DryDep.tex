...
The stomatal conductance $g_\text{STO}$ in the EMAP scheme is computed using a multiplicative ansatz that is also explained in much detail in \citet{ICP:MappingManual2017}:
%
\begin{equation}
...
\end{equation}
%
The factors herein vary within the range $0-1$ and account for leaf phenology ($f_\text{phen}$), photoactive radiation ($f_\text{light}$), temperature ($f_\text{T}$), water vapor pressure deficit ($f_\text{D}$), and soil water content ($f_\text{SW}$).
The temperature ajustment is computed from
%
\begin{equation}
...
\end{equation}
with $\beta = \frac{T_\text{max}-T_\text{opt}}{T_\text{opt}-T_\text{min}}$. The parameters $T_\text{min}$ and $T_\text{opt}$ are tabulated for various species of plants and can be found in the supplementary to \citet{ACP:Simpson2012} or in Supplement~S.2. 
%
\begin{equation}
