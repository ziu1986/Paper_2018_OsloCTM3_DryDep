...
\end{equation}
with a temperature dependent dynamic viscosity of air $\mu(T) = 6.2\cdot 10^{-8}\,\unit{kg \, m^{-1} s^{-1} K^{-1}} \cdot T$ \citep{PBL} ($\{T \in \mathbb{R} | (243.15 < T < 313.15)\,\unit{K}\}$) and substituting the air density $\rho$ using the ideal gas law. $P_0$ is the surface pressure, as $T$ the 2\,\unit{m} temperature is chosen, and $R_\text{air}$ the universal gas constant for air.
