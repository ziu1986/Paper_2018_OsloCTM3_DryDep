...
\end{figure}
%
In Fig.~\ref{fig:diff_maps}, we show the average relative difference between \emph{EMEP\_ppgsssh} and \emph{Wesely\_type} on global maps by means of surface ozone burden, dry deposition velocity and total ozone dry deposition. Surface ozone increases globally except for regions covered by tropical forest. Especially in desert regions in Africa and Asia, surface ozone increases by up to $100\,\unit{\%}$. Consistently, dry deposition velocities decrease globally (up to $100\,\unit{\%}$) in desert regions, while they increase over tropical forest. With respect to total dry deposition, the picture is a bit less clear. We find a decrease of total dry deposition of ozone in desert regions and ocean covered areas and an increase in regions covered by tropical forest, while at mid and high latitudes in both hemispheres only small changes are visible. A possible explanation to these changes especially in desert regions is the difference between the previously prescribed dry deposition velocities in the Wesely scheme and the adjusting dry deposition velocities with respect to meteorology in the EMEP scheme. We cover this in more detail in the following section.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Comparison with multi-model evaluation}
\label{subsec:model}
%
In our evaluation of results, we closly follow suggestions by \citet{ACP:Hardacre2015}. For the purpose of comparison with the multi-model mean of the therein participating Task Force on Hemispheric Transport of Air Pollution (TF HTAP) models, we also have re-grided our data to a horizontal resolution of $3^\circ\times 3^\circ$. In Section~\ref{subsubsec:zonal}, we look at zonal ditributions of $[\chem{O_3}](z_0)$, $v^\chem{O_3}_\text{DD}$, and $\sum\chem{O_3^\text{DD}}$ for all our senstivity simulations and study seasonal cycles of hemispheric ozone as well as nine land surface types which have been retrospectively seperated (Section~\ref{subsubsec:seasons}). From this, we estimate the total annual ozone dry deposition onto ocean, ice, and land surfaces and compare with results from \citet{ACP:Luhar2017}.
%
\subsubsection{Zonal distribution}
\label{subsubsec:zonal}
%
The annual zonal average with respect to surface ozone concentration (Fig.~\ref{fig:mmm_drydep}a) displays, in consistancy with Fig.~\ref{fig:diff_maps}a), a general, global increase of ozone by up to $10\,\unit{ppb}$ which is largest between $(25-50)\,\unit{^\circ N}$ including the major deserts. In the deep tropics ($5\,\unit{^\circ S}-5\,\unit{^\circ N}$), the increase is smallest ($\Delta[\chem{O_3}]\leq 5\,\unit{ppb}$). We find that the EMEP scheme further intensifies the strong asymmetry between northern and southern hemisphere as a consequence of the distribution of the continental land masses and vegetation thereon. Among the sensitivity studies, there is only a low variance. Most remarkable and as expected due to the strong decrease in prescribed dry deposition velocity over ice and snow, \emph{EMEP\_ppgssh\_ice} displays almost a doubling of surface ozone in the high Arctics compared to \emph{Wesely\_type} but affects ozone concentrations already at about $50\,\unit{^\circ}$ in both hemispheres. Neglecting any dependence on solar radiation in the stomatal conductance formulation (\emph{EMEP\_offLight}) -- or in other words allowing photosynthesis 24/7 -- generally decreases the ozone concentration by about $2\,\unit{ppb}$, while choosing soil water at deeper levels than the surface layer (\emph{EMEP\_SWVL4}) increases $[\chem{O_3}](z_0)$ slightly. Rather surprisingly, switching of phenology completely (\emph{EMEP\_offPhen}), doesn't make much difference. The scenario of differing emissions (2005 in comparison to 2014 or more specifically \emph{EMEP\_ppgs\_2005} compared to \emph{EMEP\_ppgs}), yields higher ozone concentrations in the northern hemisphere in 2005 in accordance to a reduction in sulfur and \chem{NO_x} emissions in south east Asia in later years. An opposite tendency is seen for latitudes south of $30\,\unit{^\circ N}$, propbably owing to the industrial development of countries in these regions. 

In Fig.~\ref{fig:mmm_drydep}b), we show the corresponding average dry deposition velocities, as reconstructed from the ratio between surface concentration and total dry deposition since we have no direct output of these. The $v^\chem{O_3}_\text{DD}$ in the EMEP scheme are much smaller then in the Wesely scheme and about a factor of $3$ smaller compared to the multi-model-mean for latitudes between $50\,\unit{^\circ S}$ and $70\,\unit{^\circ N}$. In the remaining latitude bands, except for \emph{EMEP\_ppgssh\_ice}, all realisations of the EMEP scheme are constantly above the multi-model-mean. This indicates, that with respect to other models, the updated dry deposition velocity above ice and snow should be considered as new standard for the OsloCTM3. The dry deposition velocities are of cause independent of the emission scenario, but display a strong dependence on $f_\text{light}$ and $f_\text{phen}$. The closer to $1$ these factores are the higher do the dry deposition velocities become (compare \emph{EMEP\_offLight}, \emph{EMEP\_offPhen}, and \emph{EMEP\_ppgssh}). That highlights the importance of a proper PAR from meteorological input in CTMs or radiative transfer in PAR in global climate models (GCM) for canopy conductance computation. Albeit, the total numbers are smaller, the shape of the zonal average dry deposition velocities from the EMEP scheme and the multi-model-mean are similar (see figure in Supplement~S.6.1). The biggest exception is the zonal band $(12-30)\,\unit{^\circ S}$, which coincide with the location of Australia and its large desert regions.

The annual total ozone dry deposition is shown in Fig.~\ref{fig:mmm_drydep}b). In accordance to the priviously decribed features, we observe a reduction of the global total ozone dry deposition to almost half of \emph{Wesely\_type} in any of the sensitivity studies. The occurence of this reduction in the zonal bands where the major deserts are located may point to a important change in $v_\text{desert}^\chem{O_3}$. Consulting the old parameter file, we find $v_\text{desert}^\chem{O_3} \equiv v_\text{tundra}^\chem{O_3} = 0.26\,\unit{cm\,s^{-1}}$, while in the EMEP scheme $v_\text{desert}^\chem{O_3}=0.05\,\unit{cm\,s^{-1}}$. Similarly, dry deposition velocities over ice and snow and ocean have been even higher in the Wesely scheme than in the original EMEP scheme ($v_\text{ice/snow}^\chem{O_3}\equiv v_\text{water}^\chem{O_3}=0.07\,\unit{cm\,s^{-1}}$). These differences in baseline dry deposition velocities over huge parts of the unvegetated surface of the Earth (see Fig.~\ref{fig:pft_landsurface}) amount to most of the quantitative change we see between the Wesely and the EMEP scheme. We further elaborate on this in the following section.
...
%
To disentangle the contributions of different regions to the global ozone budget further, we will briefly look at different projections of seasonal cycles.

In Fig.~\ref{fig:mmm_drydep_hem}, the total annual ozone dry deposition seperated into mid and high latitudes in the northern hemisphere ($(30-90)\,\unit{^\circ N}$), the tropics and subtropics ($30\,\unit{^\circ S}-30\,\unit{^\circ N}$), and the mid and high latitudes in the southern hemisphere ($(30-90)\,\unit{^\circ S}$) are shown. As reference, we have again added the multi-model-mean by \citet{ACP:Hardacre2015}. As expected, the NH mid and high latitudes display a strongly pronounced seasonal cycle, while it is less pronounced in the tropics and in the SH. The highest ozone dry deposition is found in the tropics and amounts on average to the  peak levelof dry deposition in the NH for the multi-model-mean \citep{ACP:Hardacre2015} and EMEP scheme. The average tropical ozone dry deposition in the Wesely scheme diverges by about $5\,\unit{Tg}$ in comparison to its NH maximum. The seasonal cycle in the EMEP scheme appears to be shifted towards later in the year in the NH. While the general seasonal cycle in the tropics and subtropics is not changing, we find the strongest sensitivity to the verious realisations of the EMEP scheme there, with \emph{EMEP\_offLight} and \emph{EMEP\_ppgs\_2005} the highest and the lowest, respectively. This indicates that surface ozone in the tropics may be very sensitive to changes to emissions and changes in longwave radiative transfer (e.g., cloud cover, aerosol).
...
\end{figure*}
%
As suggested by \citet{ACP:Hardacre2015}, we retrospectively separate ozone dry deposition velocities with respect to surface types. Based on CML2 dynamic land surface, we generated masks for 9 different surface types (Fig.~\ref{fig:pft_landsurface}a) and used these to select gridboxes with a high percentage of these surface types, ranging from $22\,\unit{\%}$ only for tundra in the southern hemisphere to $98\,\unit{\%}$ regarding ocean. Since we have not perform a full unfolding on the data, the results should be regarded with suspicion.
%
\begin{figure*}[t]
  \includegraphics[width=12cm]{pictures/final-total_ozone_drydepvelo_monthly_pft_nh}
  \caption{Average ozone dry deposition velocities separated by land use type and compared to results from \citep{ACP:Hardacre2015}.}
  \label{fig:mmm_drydep_season_pft}
\end{figure*}
%
The Wesely scheme uses $v^\chem{O_3}_\text{tundra/desert} = 0.26\,\unit{cm\,s^{-1}}$ \citep{JGR:Hough1991}, while in the EMEP scheme we use a base value of $v^\chem{O_3}_\text{desert} = 0.05\,\unit{cm\,s^{-1}}$ and $v^\chem{O_3}_\text{tundra} = 0.24\,\unit{cm\,s^{-1}}$. From measurements of ozone fluxes in the Sahara desert, $v^\chem{O_3}_\text{desert, day} = 0.1\,\unit{cm\,s^{-1}}$, $v^\chem{O_3}_\text{desert, night} = 0.04\,\unit{cm\,s^{-1}}$, and $\left<v^\chem{O_3}_\text{desert}\right> = 0.065\,\unit{cm\,s^{-1}}$ have been deduced \citep{AE:Gusten1995}. This implies, that ozone dry deposition over desert regions is highly overestimated in the Wesely scheme, while it may be underestimated in the EMEP scheme. Similarly, the dry deposition over water $v^\chem{O_3}_\text{water}=0.07\,\unit{cm\,s^{-1}}$ in the Wesely scheme, while in the EMEP scheme we use a base value of $v^\chem{O_3}_\text{water} = 0.05\,\unit{cm\,s^{-1}}$. $v^\chem{O_3}_\text{water} = 0.019\,\unit{cm\,s^{-1}}$ \citep{JGR:Helmig2012}. \citet{ACP:Luhar2017} found $0.018\,\unit{cm\,s^{-1}} \leq \left<v^\chem{O_3}_\text{water}\right> \leq 0.039\,\unit{cm\,s^{-1}}$.
%
\begin{table*}[t]
...
\begin{figure}[t]
  ...
  \caption{Ozone dry deposition velocities at different observation sites. Comparison between Wesely scheme and average result from all sensitivity tests to observations and multi-model-mean from \citet{ACP:Hardacre2015}.}
