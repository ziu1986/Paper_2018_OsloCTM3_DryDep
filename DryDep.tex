...
\end{equation}
with a temperature dependent absolute viscosity of air $\mu(T) = 6.2\cdot 10^{-8}\,\unit{mol \, m^{-2} s^{-1} K^{-1}} \cdot T$ \citep{PBL} ($\{T \in \mathbb{R} | (233 < T < 303)\,\unit{K}\}$) and substituting the air density $\rho$ using the ideal gas law. $P_0$ is the surface pressure, as $T$ the 2\,\unit{m} temperature is chosen, and $R_\text{air}$ the universal gas constant for air.
